...
export async function resolveStockConflicts(organisationId: string): Promise<{
  ...
  // Use Prisma findMany with where clause to find negative inventory
  const negativeInventories = await db.inventory.findMany({
    where: {
      quantity: { lt: 0 },
    },
    select: {
  ...
  let resolved = 0;
  let failed = 0;
  const negativeStocks: Array<{ inventoryId: string; productId: string; currentQty: number }> = [];
...
export async function runReconciliation(organisationId: string): Promise<{
  stockRebuilt: number;
