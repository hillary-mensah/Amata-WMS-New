...
export * from './zpl.js';
import { calculateGhanaTax, PaymentMethod } from '@nexus/types';

export interface ReceiptData {
  sale: Sale & {
    items: (SaleItem & { product: { name: string; sku: string; barcode?: string | null } })[];
    user: User;
...
function padCenter(text: string, width: number = WIDTH): string {
  const padding = Math.max(0, Math.floor((width - text.length) / 2));
  return ' '.repeat(padding) + text;
}

function padLeft(text: string, width: number = WIDTH): string {
  const padding = Math.max(0, width - text.length);
  return ' '.repeat(padding) + text;
}

function padRight(text: string, width: number = WIDTH): string {
...
function doubleHeight(text: string): string {
...
function openCashDrawer(): string {
  return '\x1b70\x00\x19\xfa';
}

export function generateBarcodeCode128(data: string): string {
  const startChar = '\x00';
  const endChar = '\x01';
...
export function generateBarcodeCode39(data: string): string {
  const encoded = data.toUpperCase().replace(/[^A-Z0-9\-\.\s\$\/\+\%]/g, '') + '*';
  return `\x1b*t${encoded}`;
}

export function generateEAN13Barcode(data: string): string {
...
export function buildReceipt(data: ReceiptData, config?: PrintConfig): string {
...
export function buildInventoryLabel(product: {
  name: string;
  sku: string;
  barcode?: string | null;
...
export async function sendToBluetoothPrinter(
...
): Promise<BluetoothPrinterResult> {
  ...
  // In production, this would use a Bluetooth serial library
  // like @abandonware/noble or react-native-ble-plx
...
export async function sendToNetworkPrinter(
  ...
  host: string,
  port: number = 9100
): Promise<NetworkPrinterResult> {
  const startTime = Date.now();
  
  console.log(`🌐 Sending to network printer: ${host}:${port}`);
  console.log(`   Data length: ${receipt.length} bytes`);
  
  // In production, use Node's net module:
...
export async function sendToUsbPrinter(
  receipt: string,
  devicePath: string
): Promise<USBPrinterResult> {
  console.log(`🔌 Sending to USB printer: ${devicePath}`);
  console.log(`   Data length: ${receipt.length} bytes`);
  
  // In production, use node-usb or serialport package
...
export function parseScanData(data: string): ScanResult {
...
export const SUPPORTED_HARDWARE: SupportedHardware = {
  printers: {
    thermal: [
      'Xprinter XP-N160I',
      'Xprinter XP-N160II',
...
export async function detectBluetoothDevices(): Promise<string[]> {
  ...
  // In production, use @abandonware/noble
  return [
    'Xprinter-N160I',
    'Rongta-RP80',
  ];
...
export async function detectNetworkPrinters(subnet?: string): Promise<string[]> {
  console.log('🔍 Scanning for network printers...');
  // In production, use mDNS discovery or scan common ports
  return [
    '192.168.1.100:9100',
    '192.168.1.101:9100',
  ];
