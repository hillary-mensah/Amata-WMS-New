...
import { buildReceipt, buildLabel } from '../printing/src/escpos.js';
...
class PrintQueue extends EventEmitter {
    ...
    getOrCreateConnection(printerId, host, port) {
        ...
        try {
            ...
            try {
                ...
                if (job.jobType === 'RECEIPT') {
                    printData = buildReceipt(job.payload);
                }
                else if (job.jobType === 'LABEL') {
                    printData = buildLabel(job.payload);
                }
                else {
                    printData = Buffer.from(job.payload);
                }
    ...
    async getPrinterStatus(printerId) {
        ...
        const conn = this.getOrCreateConnection(printer.id, printer.host || '', printer.port);
        let status = { online: false, paper: 'UNKNOWN', cover: 'UNKNOWN', errors: [] };
        try {
...
export async function queueReceipt(saleId, receiptData, priority = 5) {
...
export async function queueLabel(productId, labelData, priority = 3) {
    return printQueue.enqueue({
        id: '',
        jobType: 'LABEL',
...
export async function testPrinter(printerId) {
    const printer = await db.printer.findUnique({
        where: { id: printerId },
    });
    if (!printer) {
...
//# sourceMappingURL=print-queue.js.map