...
export async function computeInventoryFromEvents(
  inventoryId: string,
  productId: string,
  branchId: string
): Promise<number> {
...
export async function verifyDigitalTwin(
  ...
  const results: DigitalTwinResult[] = [];

  for (const inv of inventories) {
    const computedQty = await computeInventoryFromEvents(
      inv.id,
      inv.productId,
      inv.branchId
    );
...
export async function getDiscrepancyAlerts(
  organisationId: string,
  status?: DiscrepancyStatus
): Promise<DiscrepancyAlert[]> {
