...
export async function predictStockLevels(organisationId, productId, branchId, daysToPredict = 30) {
...
export async function getStockPredictionsForBranch(organisationId, branchId) {
    ...
    return predictions.sort((a, b) => {
        if (a.reorderRecommendation === 'URGENT' && b.reorderRecommendation !== 'URGENT')
            return -1;
        if (b.reorderRecommendation === 'URGENT' && a.reorderRecommendation !== 'URGENT')
            return 1;
        if (a.reorderRecommendation === 'SOON' && b.reorderRecommendation === 'NONE')
            return -1;
        return 0;
    });
}
export async function calculateExpiryRiskScores(organisationId, branchId) {
    const whereClause = {
        organisationId,
    ...
    for (const batch of batches) {
        const daysUntilExpiry = Math.ceil((batch.expiresAt.getTime() - now.getTime()) / (24 * 60 * 60 * 1000));
        let riskScore = 0;
        let riskLevel = 'LOW';
...
export async function getExpiryInsights(organisationId) {
...
export async function detectSalesAnomalies(organisationId, days = 30) {
    ...
    const dailyVariance = dailyKeys.length > 0
        ? dailyValues.reduce((sum, val) => sum + Math.pow(val - avgDailySales, 2), 0) / dailyKeys.length
        : 0;
    const dailyStdDev = Math.sqrt(dailyVariance);
    const anomalousDays = Object.entries(dailySales).filter(([_, val]) => Math.abs(val - avgDailySales) > dailyStdDev * 2);
...
async function analyzeProductPatterns(organisationId, productSales) {
    const insights = [];
...
export async function generateIntelligenceReport(organisationId, branchId) {
