...
import { AnomalyType, AnomalyStatus } from '@nexus/types';
const EXCESS_REFUNDS_PER_HOUR = 3;
const EXCESS_VOIDS_PER_HOUR = 5;
const MAX_DISCOUNT_PERCENT = 25;
const HIGH_CASH_THRESHOLD = 5000;
export async function detectExcessRefunds(organisationId, userId, hours = 1) {
...
export async function detectDiscountAbuse(organisationId, branchId, userId) {
...
export async function detectSuspiciousVoids(organisationId, userId) {
    const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
    ...
    if (voidedSales.length >= EXCESS_VOIDS_PER_HOUR) {
        const totalVoided = voidedSales.reduce((sum, s) => sum + Number(s.totalAmount), 0);
        const sameItemVoids = await db.$queryRaw `
...
export async function detectHighCashTransaction(organisationId, saleId, amount, paymentMethod) {
...
export async function detectOffHoursSale(organisationId, saleId, saleTime) {
...
export async function checkNegativeStockSale(organisationId, saleId) {
...
export async function analyzeCashierBehavior(organisationId, userId, days = 7) {
    const anomalies = [];
...
export async function runAnomalyDetection(organisationId, context) {
    const anomalies = [];
    if (context.userId) {
        ...
        if (context.branchId) {
            const discountAnomaly = await detectDiscountAbuse(organisationId, context.branchId, context.userId);
            if (discountAnomaly)
                anomalies.push(discountAnomaly);
        }
    }
...
export async function createAnomaly(organisationId, anomaly) {
...
export async function resolveAnomaly(anomalyId, status, notes) {
